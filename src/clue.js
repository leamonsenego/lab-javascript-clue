...
function selectRandom(suspect, room, weapon) {
  const suspectIndex = Math.floor(Math.random() * suspect.length);
  const roomIndex = Math.floor(Math.random() * room.length);
  const weaponsIndex = Math.floor(Math.random() * weapon.length);

  const randomSuspectFirstName = suspect[suspectIndex].firstName;
  const randomSuspectLastName = suspect[suspectIndex].lastName;
  const randomRoom = room[roomIndex].name;
  const randomWeapon = weapon[weaponsIndex].name;

  console.log(`${randomSuspectFirstName} ${randomSuspectLastName} killed Mr Body in the ${randomRoom} with a ${randomWeapon}.`)
